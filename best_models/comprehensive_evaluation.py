...
def create_pipeline(model_type, use_fs, impute_strategy, scale_method, model_params=None):
...
def get_model_parameters_interactive(model_type):
    ...
    if model_type == 'HGB':
        ...
        if lr: params['learning_rate'] = float(lr)
        
        depth = input(f"  Max depth [default: 5]: ").strip()
        if depth: params['max_depth'] = int(depth)
        
        leaf = input(f"  Min samples leaf [default: 50]: ").strip()
        if leaf: params['min_samples_leaf'] = int(leaf)
        
        l2 = input(f"  L2 regularization [default: 1.0]: ").strip()
        if l2: params['l2_regularization'] = float(l2)
        
    elif model_type == 'RF':
        trees = input(f"  Number of trees [default: 300]: ").strip()
        if trees: params['n_estimators'] = int(trees)
        
        depth = input(f"  Max depth [default: 15, None for unlimited]: ").strip()
        if depth: params['max_depth'] = None if depth.lower() == 'none' else int(depth)
        
        split = input(f"  Min samples split [default: 10]: ").strip()
        if split: params['min_samples_split'] = int(split)
        ...
        if penalty: params['penalty'] = penalty
...
def evaluate_model(model_type, features, labels, trainFeatures, trainLabels, 
                   testFeatures, testLabels, use_fs, impute_strategy, 
                   scale_method, model_params, n_runs=3):
    ...
    for run in range(n_runs):
        # Create pipeline
        pipeline = create_pipeline(model_type, use_fs, impute_strategy, scale_method, model_params)
...
def generate_learning_curves(models_config, features, labels, use_fs, impute_strategy, scale_method, output_folder):
    ...
    for model_type, model_params in models_config.items():
        ...
        sizes, train_scores_auc, val_scores_auc = learning_curve(
            pipeline, features, labels, cv=cv, n_jobs=-1,
        ...
        ax1.plot(sizes, train_auc_mean, 'o-', color='red', linewidth=2.5, 
                label='Training AUC', markersize=8)
        ax1.fill_between(sizes, train_auc_mean - train_auc_std, train_auc_mean + train_auc_std,
                        alpha=0.2, color='red')
        
        ax1.plot(sizes, val_auc_mean, 'o-', color='green', linewidth=2.5, 
                label='Validation AUC', markersize=8)
        ax1.fill_between(sizes, val_auc_mean - val_auc_std, val_auc_mean + val_auc_std,
                        alpha=0.2, color='green')
        
        ax1.set_xlabel('Training Examples', fontsize=11)
        ax1.set_ylabel('AUC Score', fontsize=11)
        ax1.set_title(f'AUC Learning Curve - {model_type}', fontsize=12, fontweight='bold')
        ...
        ax1.text(0.02, 0.02, f'Final Train: {final_train_auc:.3f}\nFinal Val: {final_val_auc:.3f}',
                transform=ax1.transAxes, fontsize=9, verticalalignment='bottom',
                bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
        
        # Top Right: Accuracy Learning Curves
        ax2.plot(sizes, train_acc_mean, 'o-', color='darkred', linewidth=2.5, 
                label='Training Accuracy', markersize=8)
        ax2.fill_between(sizes, train_acc_mean - train_acc_std, train_acc_mean + train_acc_std,
                        alpha=0.2, color='darkred')
        
        ax2.plot(sizes, val_acc_mean, 'o-', color='darkgreen', linewidth=2.5, 
                label='Validation Accuracy', markersize=8)
        ax2.fill_between(sizes, val_acc_mean - val_acc_std, val_acc_mean + val_acc_std,
                        alpha=0.2, color='darkgreen')
        
        ax2.set_xlabel('Training Examples', fontsize=11)
        ax2.set_ylabel('Accuracy', fontsize=11)
        ax2.set_title(f'Accuracy Learning Curve - {model_type}', fontsize=12, fontweight='bold')
        ...
        ax2.text(0.02, 0.02, f'Final Train: {final_train_acc:.3f}\nFinal Val: {final_val_acc:.3f}',
        ...
        ax3.plot(sizes, gap_auc, 'o-', color='orange', linewidth=2.5, 
                label='AUC Gap', markersize=8)
        ax3.fill_between(sizes, 0, gap_auc, alpha=0.3, color='orange')
        ax3.axhline(y=0, color='k', linestyle='--', linewidth=1, alpha=0.5)
        ax3.axhline(y=0.05, color='green', linestyle=':', linewidth=1.5, alpha=0.7,
                   label='Good fit')
        ax3.axhline(y=0.15, color='red', linestyle=':', linewidth=1.5, alpha=0.7,
                   label='Overfitting')
        ...
        ax3.text(0.98, 0.98, f'Gap: {final_gap_auc:.3f}\n{status_auc}',
        ...
        ax4.plot(sizes, gap_acc, 'o-', color='purple', linewidth=2.5, 
                label='Accuracy Gap', markersize=8)
        ax4.fill_between(sizes, 0, gap_acc, alpha=0.3, color='purple')
        ax4.axhline(y=0, color='k', linestyle='--', linewidth=1, alpha=0.5)
        ax4.axhline(y=0.05, color='green', linestyle=':', linewidth=1.5, alpha=0.7,
                   label='Good fit')
        ax4.axhline(y=0.15, color='red', linestyle=':', linewidth=1.5, alpha=0.7,
                   label='Overfitting')
        ...
        ax4.text(0.98, 0.98, f'Gap: {final_gap_acc:.3f}\n{status_acc}',
        ...
        plt.suptitle(f'{model_type} - Training vs Validation Performance', 
                    fontsize=14, fontweight='bold', y=0.995)
        plt.tight_layout()
        filename = os.path.join(output_folder, f"learning_curve_{model_type.lower()}.png")
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        print(f"  ✓ Saved: {filename}")
        print(f"  AUC gap: {final_gap_auc:.4f} ({status_auc}) | Accuracy gap: {final_gap_acc:.4f} ({status_acc})")
        plt.close()
        
        all_gaps.append({'model': model_type, 'gap': final_gap, 'color': colors[model_type]})
    ...
    ax.axhline(y=0.05, color='green', linestyle='--', linewidth=2, alpha=0.7, label='Good fit')
    ax.axhline(y=0.15, color='orange', linestyle='--', linewidth=2, alpha=0.7, label='Mild overfitting')
    
    ax.set_ylabel('Train-Validation AUC Gap', fontsize=12)
    ax.set_xlabel('Model', fontsize=12)
    ax.set_title('Overfitting Comparison Across All Models', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
...
def plot_comparison(results, config_name, output_folder):
    ...
    ax1 = axes[0]
    for r in results:
        ax1.plot(r['fpr'], r['tpr'], color=colors[r['model']], linewidth=2.5,
    ...
    for r in results:
        mask = r['fpr'] <= 0.1
        ax2.plot(r['fpr'][mask], r['tpr'][mask], color=colors[r['model']], linewidth=2.5,
    ...
    bars1 = ax3.bar(x - width/2, test_aucs, width, label='Test AUC',
                    color=model_colors, alpha=0.7, edgecolor='black')
    bars2 = ax3.bar(x + width/2, tprs, width, label='TPR @ FPR=0.01',
                    color=model_colors, alpha=0.4, edgecolor='black')
...
def main():
    ...
    for model_type in models_to_run:
        ...
        result = evaluate_model(
            model_type, features, labels, trainFeatures, trainLabels,
            testFeatures, testLabels, use_fs, impute_strategy,
            scale_method, models_config[model_type], n_runs
        )
        results.append(result)
        print(f"  CV AUC: {result['cv_mean']:.4f} ± {result['cv_std']:.4f}")
        print(f"  Test AUC: {result['test_auc_mean']:.4f} ± {result['test_auc_std']:.4f}")
    ...
    generate_learning_curves(models_config, features, labels, use_fs, impute_strategy, scale_method, output_folder)
