...
#table(
  ...
  inset: 10pt,
  align: horizon,
  table.header([Approach], [Best Model], [Test AUC], [CV Mean $+/-$ Std], [Key Insight]),
  [Original],
  [SelectKBest + HistGradientBoosting],
  [$0.898$ ],
  [ $0.908 +/- 0.024$ ],
  [Feature selection improves performance ],

  [RF Selection],
  [RF-selected + HistGradientBoosting],
  [$0.898$ ],
  [ $0.908 ± 0.024$ ],
  [Feature selection improves performance ],

  [ Combined ],
  [Robust + SelectKBest + HistGradientBoosting],
  [$0.897$],
  [ $0.909 +/- 0.022$],
  [Proper preprocessing pipelines work well],
)

*Final Recommendation*

Use HistGradientBoosting with Random Forest-based feature selection, as it
provides the best balance of performance, efficiency, and interpretability for
this spam classification task.
...
+ *Feature Selection Strategy*: Random Forest's MDI proved more effective than mutual information or L1 regularization for this dataset, likely because it captures non-linear feature interactions that are crucial for spam detection.

+ *Preprocessing Order*: The correct sequence (imputation → scaling → feature selection → modeling) was critical for optimal performance.

+ *Model Robustness*: HistGradientBoosting's performance remained consistent across different preprocessing combinations, indicating its robustness to feature scaling and imputation strategies.

+ *Computational Efficiency*: Reducing from 30 to 20 features improved training time while maintaining or slightly improving performance, demonstrating the value of principled feature selection.
