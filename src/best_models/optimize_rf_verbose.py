...
def strategy_feature_selection_variants(features, labels, cv):
    ...
    scores = cross_val_score(
        pipeline_no_fs, features, labels, cv=cv, scoring="roc_auc", n_jobs=-1
    )
    results.append(
        {
            "variant": "No_FS",
    ...
    start = time.time()
    scores = cross_val_score(
        pipeline_sfm_median, features, labels, cv=cv, scoring="roc_auc", n_jobs=-1
    )
    results.append(
        {
            "variant": "SFM_median",
    ...
    start = time.time()
    scores = cross_val_score(
        pipeline_sfm_mean, features, labels, cv=cv, scoring="roc_auc", n_jobs=-1
    )
    results.append(
        {
            "variant": "SFM_mean",
    ...
    start = time.time()
    scores = cross_val_score(
        pipeline_kbest, features, labels, cv=cv, scoring="roc_auc", n_jobs=-1
    )
    results.append(
        {
            "variant": "KBest_50pct",
    ...
    start = time.time()
    scores = cross_val_score(
        pipeline_sfm_aggressive, features, labels, cv=cv, scoring="roc_auc", n_jobs=-1
    )
    results.append(
        {
...
def evaluate_final_models(strategies, features, labels, cv, output_folder):
    ...
    for strategy in strategies:
        ...
        cv_scores = cross_val_score(
            strategy["pipeline"], features, labels, cv=cv, scoring="roc_auc", n_jobs=-1
        )
    ...
    for i, r in enumerate(results, 1):
        print(
            f"{i:<6} {r['name']:<30} {r['cv_auc_mean']:.4f} ± {r['cv_auc_std']:.4f}    {r['tpr_at_fpr001']:.4f}"
        )
    ...
    results_df.to_csv(
        os.path.join(output_folder, "optimization_results.csv"), index=False
    )
...
def plot_results_comparison(results, output_folder):
    ...
    # Print optimized parameters for best model BEFORE plotting
    print("\n" + "=" * 70)
    print("BEST MODEL OPTIMIZED PARAMETERS")
    print("=" * 70)
    best_model = results[0]
    print(f"\nStrategy: {best_model['name']}")
    print(f"CV AUC: {best_model['cv_auc_mean']:.4f} ± {best_model['cv_auc_std']:.4f}")
    print(f"TPR @ FPR=0.01: {best_model['tpr_at_fpr001']:.4f}")
    print("\nOptimized Hyperparameters:")
    print("-" * 70)
    if best_model["params"]:
        for param, value in sorted(best_model["params"].items()):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_folder, "rf_optimization_results.png"),
        dpi=300,
        bbox_inches="tight",
    )
    print(f"✓ Visualization saved to {output_folder}/rf_optimization_results.png")
    plt.close()
...
def main():
    ...
    final_results = evaluate_final_models(
        all_strategies, features, labels, cv, output_folder
    )
    ...
    print(
        f"  CV AUC: {final_results[0]['cv_auc_mean']:.4f} ± {final_results[0]['cv_auc_std']:.4f}"
    )
    print(f"  TPR @ FPR=0.01: {final_results[0]['tpr_at_fpr001']:.4f}")

    # Print the optimized parameters one final time at the end
    print("\n" + "=" * 70)
    print("FINAL OPTIMIZED PARAMETERS")
    print("=" * 70)
    if final_results[0]["params"]:
        for param, value in sorted(final_results[0]["params"].items()):
            print(f"  {param:.<50} {value}")
    else:
        print("  (Using default configuration)")
    print("=" * 70)
