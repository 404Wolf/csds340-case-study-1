...
model_l1_no_tfidf = LogisticRegression(
    penalty="l1", C=10.0, solver="liblinear", max_iter=10000, random_state=1
)
model_l1_no_tfidf.fit(train_features, train_labels)

# Train L1 models - with TF-IDF
model_l1_with_tfidf = LogisticRegression(
    penalty="l1", C=10.0, solver="liblinear", max_iter=10000, random_state=1
)
...
test_probs_bernoulli_no_tfidf = model_bernoulli_no_tfidf.predict_proba(test_features)[
    :, 1
]
...
test_probs_bernoulli_with_tfidf = model_bernoulli_with_tfidf.predict_proba(
    test_features_tfidf
)[:, 1]
test_preds_bernoulli_with_tfidf = model_bernoulli_with_tfidf.predict(
    test_features_tfidf
)
...
print(
    f"  Non-zero coefficients: {np.sum(model_l1_no_tfidf.coef_ != 0)}/{len(model_l1_no_tfidf.coef_[0])}"
)
print(f"\nL1 with TF-IDF:")
print(f"  Accuracy: {acc_with_tfidf:.4f}")
print(f"  AUC: {auc_with_tfidf:.4f}")
print(
    f"  Non-zero coefficients: {np.sum(model_l1_with_tfidf.coef_ != 0)}/{len(model_l1_with_tfidf.coef_[0])}"
)
...
fpr_bernoulli_no_tfidf, tpr_bernoulli_no_tfidf, _ = roc_curve(
    test_labels, test_probs_bernoulli_no_tfidf
)
fpr_bernoulli_with_tfidf, tpr_bernoulli_with_tfidf, _ = roc_curve(
    test_labels, test_probs_bernoulli_with_tfidf
)
...
partial_auc_bernoulli_no_tfidf = partial_auc(
    fpr_bernoulli_no_tfidf, tpr_bernoulli_no_tfidf
)
partial_auc_bernoulli_with_tfidf = partial_auc(
    fpr_bernoulli_with_tfidf, tpr_bernoulli_with_tfidf
)
...
axes[0].plot(
    ...
    "b-",
    linewidth=2,
    label=f"L1 without TF-IDF (AUC={auc_no_tfidf:.3f})",
)
axes[0].plot(
    fpr_with_tfidf,
    tpr_with_tfidf,
    "r-",
    linewidth=2,
    label=f"L1 with TF-IDF (AUC={auc_with_tfidf:.3f})",
)
axes[0].plot(
    fpr_bernoulli_no_tfidf,
    tpr_bernoulli_no_tfidf,
    "g-",
    linewidth=2,
    label=f"Bernoulli NB without TF-IDF (AUC={auc_bernoulli_no_tfidf:.3f})",
)
axes[0].plot(
    fpr_bernoulli_with_tfidf,
    tpr_bernoulli_with_tfidf,
    "m-",
    linewidth=2,
    label=f"Bernoulli NB with TF-IDF (AUC={auc_bernoulli_with_tfidf:.3f})",
)
...
axes[1].plot(
    ...
    "b-",
    linewidth=2,
    label=f"L1 without TF-IDF (pAUC={partial_auc_no_tfidf:.4f})",
)
axes[1].plot(
    fpr_with_tfidf,
    tpr_with_tfidf,
    "r-",
    linewidth=2,
    label=f"L1 with TF-IDF (pAUC={partial_auc_with_tfidf:.4f})",
)
axes[1].plot(
    fpr_bernoulli_no_tfidf,
    tpr_bernoulli_no_tfidf,
    "g-",
    linewidth=2,
    label=f"Bernoulli NB without TF-IDF (pAUC={partial_auc_bernoulli_no_tfidf:.4f})",
)
axes[1].plot(
    fpr_bernoulli_with_tfidf,
    tpr_bernoulli_with_tfidf,
    "m-",
    linewidth=2,
    label=f"Bernoulli NB with TF-IDF (pAUC={partial_auc_bernoulli_with_tfidf:.4f})",
)
