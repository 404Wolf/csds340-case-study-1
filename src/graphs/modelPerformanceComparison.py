...
def optimize_and_evaluate_model(
    ...
    if not optimize:
        ...
        cv_scores = cross_val_score(
            pipeline, train_features, train_labels, cv=cv, scoring="roc_auc", n_jobs=-1
        )
    ...
    if model_type not in param_grids:
        ...
        return optimize_and_evaluate_model(
            model_type,
            train_features,
            train_labels,
            test_features,
...
def create_model_performance_graph():
    ...
    print(
        f"Train: {train_features.shape[0]} samples, {train_features.shape[1]} features"
    )
    ...
    bars2 = ax1.bar(x + width / 2, test_aucs, width, label="Test AUC", alpha=0.8)
...
def create_learning_curves_graph():
    ...
    print(
        f"Training dataset: {features.shape[0]} samples, {features.shape[1]} features"
    )
