...
def analyze_feature_overlap(trainFeatures, trainLabels, drop_percentages, c_values):
    ...
    for drop_pct in drop_percentages:
        ...
        rf_dropped, rf_sorted, rf_importance = get_rf_dropped_features(train_imputed, trainLabels, drop_pct)
        ...
        for C in c_values:
            try:
                l1_dropped, l1_preserved = get_l1_dropped_features(train_imputed, trainLabels, C)
        ...
        print(f"  RF dropped: {n_rf_dropped}, L1 dropped: {best_n_l1_dropped} (C={best_c:.3f})")
        print(f"  Common dropped: {n_common}, Overlap: {overlap_pct:.1f}%, Jaccard: {jaccard:.3f}")
...
def plot_feature_overlap(overlap_data):
    ...
    ax1.plot(drop_percentages, overlap_data['rf_dropped_counts'], 'o-', 
             label='Random Forest', color='tab:blue', linewidth=2, markersize=6)
    ax1.plot(drop_percentages, overlap_data['l1_dropped_counts'], 's-', 
             label='L1 Regularization', color='tab:red', linewidth=2, markersize=6)
    ax1.plot(drop_percentages, overlap_data['common_dropped_counts'], '^-', 
             label='Common Dropped', color='tab:green', linewidth=2, markersize=6)
    ...
    ax2.plot(drop_percentages, overlap_data['overlap_percentages'], 'o-', 
    ...
    ax3.plot(drop_percentages, overlap_data['jaccard_similarities'], 'o-', 
    ...
    ax4.semilogx(drop_percentages, overlap_data['c_values'], 'o-', 
                 color='tab:brown', linewidth=2, markersize=6)
...
def plot_overlap_superposition(overlap_data):
    ...
    line1 = ax1.plot(drop_percentages, overlap_data['overlap_percentages'], 'o-', 
                     color=color1, label='Overlap Percentage', linewidth=3, markersize=8)
    ...
    line2 = ax2.plot(drop_percentages, overlap_data['jaccard_similarities'], 's-', 
                     color=color2, label='Jaccard Similarity', linewidth=3, markersize=8)
    ax2.tick_params(axis='y', labelcolor=color2)
    ax2.set_ylim(0, 1)
    
    # Add title and combined legend
    plt.title('Feature Dropping Agreement: Random Forest vs L1 Regularization\\n' +
              'Superposition of Overlap Metrics', fontsize=16, pad=20)
...
def main():
    ...
    print(f"Training set: {trainFeatures.shape[0]} samples, {trainFeatures.shape[1]} features")
    ...
    overlap_data = analyze_feature_overlap(trainFeatures, trainLabels, drop_percentages, c_values)
    ...
    print("\\nSummary Statistics:")
    print(f"Average overlap percentage: {np.mean(overlap_data['overlap_percentages']):.1f}%")
    print(f"Maximum overlap percentage: {np.max(overlap_data['overlap_percentages']):.1f}%")
    print(f"Minimum overlap percentage: {np.min(overlap_data['overlap_percentages']):.1f}%")
    print(f"Average Jaccard similarity: {np.mean(overlap_data['jaccard_similarities']):.3f}")
    print(f"Maximum Jaccard similarity: {np.max(overlap_data['jaccard_similarities']):.3f}")
    
    # Detailed results table
    print(f"\\nDetailed Results:")
    print(f"{'Drop%':<6} {'RF_Drop':<8} {'L1_Drop':<8} {'Common':<7} {'Overlap%':<8} {'Jaccard':<8} {'C_Value':<10}")
    print("-" * 65)
    for i in range(len(overlap_data['drop_percentages'])):
        print(f"{overlap_data['drop_percentages'][i]:<6} "
